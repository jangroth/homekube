...
- [ ] **Consolidate documentation** — once all specs (005–007+) are implemented, do a pass to merge the scattered component-version tables, outdated README sections, and phase docs into a coherent state. `homekube-main/README.md` Components table is a known stale spot. Make sure to incorporate the resource budget from spec-005-production-cluster-setup.
...
- [x] **Deploy Homepage application dashboard** — done via spec 007 (2026-07-03): v1.13.2 on LB VIP `192.168.86.245`, vendored raw manifests, ArgoCD/Longhorn/Prometheus widgets live, Grafana link-only (DECISION-044), open on VIP (DECISION-043)

- [ ] **Homepage follow-ups** — add a Tailscale widget (Networking group; needs a tailnet API key → sealed-secret). MySpeed considered and dropped (would need self-hosting). Back-fill `gethomepage.dev/*` discovery annotations onto existing LB Services if annotation discovery is wanted (spec 007 §4, optional).

- [ ] **Offline homekube-CA for from-scratch provisioning (spec-worthy, ~008)** — the CA keypair is currently minted *inside* the cluster (`selfSigned` bootstrap issuer → `homekube-ca-secret`), so a DR rebuild generates a new CA and silently invalidates every PEM copy committed to git: ArgoCD OIDC `rootCA` (argocd-helm-values.yaml), homepage `ca-configmap.yaml`, control-node browser-trust cert. Fix: generate the CA offline once (key in password manager, cert in git), replace the selfSigned bootstrap with a `ca:` ClusterIssuer fed from the offline keypair (sealed-secret re-sealed on rebuild, or Ansible bootstrap task), and migrate the live cluster (issuer swap → certs re-issue → re-import browser trust). Until then, DR requires: extract new CA from `homekube-ca-secret` → refresh all three PEM copies → re-run `50-gitops` + `20-configure-darth`. Interim state (2026-07-03): `homekube-ca-secret` full YAML is already saved in the password manager (see Phase 5 human step), so a rebuild *can* restore the original CA by re-creating that secret before cert-manager syncs — document this as the stopgap DR path. (Discussed during spec 007 implementation.)
